...
var guestbookAdmin = {
  ...
  */
  processResponse : function () {
    ...
    if (response) {
      if (response.value == '1') {
        if (response.id == 'all') $$('.comment').each(function(el){el.remove()});
        else $(response.id).remove();
  ...
  */
  initialize : function () {
    this.xhrReq = new XHR({'method':'get', 'onSuccess':this.processResponse.bind(this), 'onFailure':this.responseFailure.bind(this)});
    $$('.mark_spam, .mark_ham, .delete_comment, #delete_spam').each(function (el) {
      el.addEvent('click', function(e){
        if (!this.xhrReq.running) this.xhrReq.send(el.getProperty('href'), '_ajax=1');
