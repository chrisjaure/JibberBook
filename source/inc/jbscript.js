...
var Guestbook = {
  ...
  processAddResponse : function () {
    ...
    if (response) {
      if (response.value != '1') {
        ...
        if (response.value == '2') this.obj.form.reset();
      }
      else {
        ...
        if (this.obj.scroller == window) {
          this.fx.scroll.toElement(this.obj.commentWrapper).chain(function(){this.showMessage(response.message, 'confirm');}.bind(this));
        } else {
    ...
    } else this.showMessage('An error has occurred.', 'error');
    this.loaded();
  },
	...
	*/
    ...
    if (response.value == '0') {
      ...
      this.obj.loadingMessage.setText('All comments have been loaded.');
    }
  ...
  */
	loading : function () {
		this.obj.submitButton.setProperties({'disabled': true, 'value': 'Loading...'});
	},
	...
	*/
	loaded : function () {
		this.obj.submitButton.setProperties({'disabled': false, 'value': this.obj.submitButton.orgVal});
	},
  ...
  */
	initialize : function (form, comments, message, loading) {
    ...
    // set effects
		...
		this.fx.scroll = new Fx.Scroll(this.obj.scroller, {'duration': 1000, 'transition': Fx.Transitions.Circ.easeInOut});
    
    // set hxr objects 
    this.loadComments = new XHR({method: 'get', 'onSuccess': this.processLoadComments.bind(this)});
    this.ajaxReq = new XHR({method: 'post', 'onSuccess': this.processAddResponse.bind(this), 'onFailure': function () {
			this.showMessage('Your comment could not be added. Please try again later.', 'error');
			this.loaded();
		}.bind(this)});
    ...
    if (this.obj.loadingMessage) {
      this.obj.loadingMessage.setText('More comments are loading. If you are using the scrollbar, please release your mouse.');
    ...
    // misc
    if (window.ie6) this.obj.messageWrapper.setStyle('zoom', 1); // fix background issue in ie6
	}
};
